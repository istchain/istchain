...
func newShardCmd(opts ethermintserver.StartOptions) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: `shard opens a local istchain home directory's databases and removes all blocks outside a range defined by --start and --end. The range is inclusive of the end block.
...
WARNING: this is a destructive action.`,
		Example: `Create a 1M block data shard (keeps blocks istchain 1,000,000 to 2,000,000)
$ istchain shard --home path/to/.istchain --start 1000000 --end 2000000

Prune all blocks up to 5,000,000:
$ istchain shard --home path/to/.istchain --start 5000000 --end -1

Prune first 1M blocks _without_ affecting blockstore or cometBFT state:
$ istchain shard --home path/to/.istchain --start 1000000 --end -1 --only-app-state`,
