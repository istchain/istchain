...
import (
	...
	testutil "github.com/istchain/istchain/x/istdist/testutil"
...
func (suite *genesisTestSuite) TestInitGenesis_ValidationPanic() {
	...
	suite.Require().Panics(func() {
		istdist.InitGenesis(suite.Ctx, suite.Keeper, suite.AccountKeeper, invalidState)
...
func (suite *genesisTestSuite) TestInitAndExportGenesis() {
	...
	istdist.InitGenesis(suite.Ctx, suite.Keeper, suite.AccountKeeper, state)
	suite.Require().Equal(state.Params, suite.Keeper.GetParams(suite.Ctx))

	exportedState := istdist.ExportGenesis(suite.Ctx, suite.Keeper)
