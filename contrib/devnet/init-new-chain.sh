...
#        istchain1ffv7nhd3z6sych2qpqkk03ec6hzkmufy0r2s4c
# istchainvaloper1ffv7nhd3z6sych2qpqkk03ec6hzkmufyz4scd0
...
# istchain1adkm6svtzjsxxvg7g6rshg6kj9qwej8gwqadqd
...
# istchain18jz5lyhhy6ncjlyty064kttw93yzaulq7rlptu
...
# istchain10wlnqzyss4accfqmyxwx5jy5x9nfkwh6qm7n4t
...
# istchain15tmj37vh7ch504px9fcfglmvx6y9m70646ev8t
...
chainID="istchainlocalnet_8888-1"
...
# Set client chain id
sed -in-place='' 's/chain-id = ""/chain-id = "istchainlocalnet_8888-1"/g' $DATA/config/client.toml
...
$BINARY add-genesis-account $validatorKeyName 2000000000uist,100000000000bnb
...
$BINARY add-genesis-account $faucetKeyName 1000000000uist,100000000000bnb

evmFaucetKeyName="evm-faucet"
printf "$evmFaucetMnemonic\n" | $BINARY keys add $evmFaucetKeyName --eth --recover
$BINARY add-genesis-account $evmFaucetKeyName 1000000000uist

userKeyName="user"
printf "$userMnemonic\n" | $BINARY keys add $userKeyName --eth --recover
$BINARY add-genesis-account $userKeyName 1000000000uist,1000000000usdx

relayerKeyName="relayer"
printf "$relayerMnemonic\n" | $BINARY keys add $relayerKeyName --eth --recover
$BINARY add-genesis-account $relayerKeyName 1000000000uist

# Create a delegation tx for the validator and add to genesis
$BINARY gentx $validatorKeyName 1000000000uist --keyring-backend test --chain-id $chainID
$BINARY collect-gentxs

# Replace stake with uist
sed -in-place='' 's/stake/uist/g' $DATA/config/genesis.json

# Replace the default evm denom of aphoton with uist
sed -in-place='' 's/aphoton/aist/g' $DATA/config/genesis.json
...
jq '.app_state.savings.params.supported_denoms = ["bist-istchainvaloper1ffv7nhd3z6sych2qpqkk03ec6hzkmufyz4scd0"]' $DATA/config/genesis.json | sponge $DATA/config/genesis.json
