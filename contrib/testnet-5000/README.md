...
Testnet-5000 introduces transfers between IstChain and Bnbchain via BEP3.

This guide will walk you through interacting with the blockchains and transferring coins via the rest server. To send transactions, we'll create an unsigned request, sign it, and broadcast it to the IstChain blockchain.

## Rest server requests

### Setup

We'll be using IstChain's CLI to build, sign, and broadcast the transactions:
...
```bash
    istcli q auth account $(istcli keys show testuser -a)
```
...
```bash
    ...
    # Sign the request
    istcli tx sign ./contrib/testnet-5000/rest_examples/create-swap-unsigned.json --from testnetdeputy --offline --chain-id testing --sequence 0 --account-number 5 | jq > ./contrib/testnet-5000/rest_examples/broadcast-create-swap.json

    # Broadcast the request
    istcli tx broadcast ./contrib/testnet-5000/rest_examples/broadcast-create-swap.json
```
...
Congratulations, you've just created a swap on IstChain! The swap will be automatically relayed over to Bnbchain where it it can be claimed using the secret random number from above.
...
```bash
    ...
    # Sign the request
    istcli tx sign ./contrib/testnet-5000/rest_examples/claim-swap-unsigned.json --from user --offline --chain-id testing --sequence 0 --account-number 1 | jq  > ./contrib/testnet-5000/rest_examples/broadcast-claim-swap.json

    # Broadcast the request
    istcli tx broadcast ./contrib/testnet-5000/rest_examples/broadcast-claim-swap.json
```
...
```bash
    ...
    # Sign the request
    istcli tx sign ./contrib/testnet-5000/rest_examples/refund-swap-unsigned.json --from user --offline --chain-id testing --sequence 0 --account-number 1 | jq  > ./contrib/testnet-5000/rest_examples/broadcast-refund-swap.json

    # Broadcast the request
    istcli tx broadcast ./contrib/testnet-5000/rest_examples/broadcast-refund-swap.json
```
